...
### Added
- **OKF profile for `.specs/`** — the generated bundle is now an [Open Knowledge Format](https://cloud.google.com/blog/products/data-analytics/how-the-open-knowledge-format-can-improve-data-sharing) knowledge graph: a controlled `type` vocabulary + YAML frontmatter (`status`, `work_item`, `work_type`, `traces`) on all templates. See `doc/design/okf-perfil-sddk.md`
- **Status lifecycle** (`draft → approved → implemented → verified`) wired into the 5 skills; the spec document is the anchor artifact
- **Native Claude Code plugin** — `sddk/.claude-plugin/plugin.json` + repo-root `.claude-plugin/marketplace.json` (the repo doubles as a single-plugin marketplace)
- **`sddk:verifier` subagent** — read-only independent verification (judges the implementation against spec/SDD/manual-tests, never edits), invoked by Code Review before `verified`
- **Pipeline-gate hooks** (`sddk/hooks/`) — `Stop` blocks implemented-but-unverified work; `PreToolUse` advises on premature code writes. Fail-open, cross-platform Node
- **`log.md`** — OKF-reserved project changelog, split out of the project index
- **Progressive disclosure** — the Dev skill traverses the OKF graph via `traces`/pointers instead of loading whole documents (`doc/design/progressive-disclosure-poc.md`)
- **9 new validation tests** (Layers 5–7: plugin manifests, read-only verifier, functional hook gate) — 27 total

### Changed
- **`.specs/features/_overview.md` → `.specs/index.md`** (`type: project-index`) at the OKF bundle root
- **Install (Claude Code)** — the native plugin is now the recommended method; the npm CLI copy-install remains for Gemini and as a Claude alternative

### Breaking
- The generated `.specs/` artifact structure changed (mandatory frontmatter, `index.md`/`log.md` relocation). Projects generated by SDDK ≤ 2.5.0 keep working, but new artifacts follow the OKF profile
