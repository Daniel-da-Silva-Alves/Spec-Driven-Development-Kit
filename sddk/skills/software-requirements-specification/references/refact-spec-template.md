...
> **OKF profile (mandatory).** Every generated document begins with the YAML frontmatter below. It makes the `.specs/` bundle a portable, machine-readable knowledge graph and is what the pipeline gates read (via `status`). See the profile contract in `doc/design/okf-perfil-sddk.md`. Fill the frontmatter FIRST, then the body. The document body follows IEEE 1219 / ISO 14764 unchanged.
...
---
type: refact-spec
title: "Refactoring Specification — {Refactoring Name}"
description: "{one-line summary of the refactoring motivation}"
...
8. **OKF frontmatter is mandatory** — always emit it, with `type: refact-spec` and `status: draft` on creation. Set `status: approved` only after the user approves the document. `work_item` MUST match the refact folder name exactly
